...
- **Relax the existing `club_memberships_sponsor_check` constraint** so that `sponsor_member_id` can be `NULL` for memberships in `applying`, `submitted`, `interview_scheduled`, `interview_completed`, `declined`, and `withdrawn` states. Cold applications have no sponsor. For invitation-backed memberships, `sponsor_member_id` is populated from the invitation sponsor.
...
- Relax `club_memberships_sponsor_check` so application-state memberships allow `NULL` sponsor.
