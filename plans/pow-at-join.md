...
5. **IPv6 `/64` keying for the anonymous bucket.** Tracked separately in `HARDENING.md` §2.4 and `PEN_TESTING.md` §P0.5. Not in scope for this workstream, but worth naming here: pow-at-join only keeps attackers honest if the `anon_join:${ip}` bucket is real. Full-address IPv6 keying is trivially bypassed — a residential customer's `/64` allocation is 2^64 addresses. Until the rate-limit key uses `/64` for IPv6, pow-at-join's cost floor is CPU-per-solve, not CPU-per-solve-per-real-human. Ship pow-at-join anyway; it is a strict improvement. But do not treat the anonymous wall as durably defended until `/64` keying AND shared-state rate limits have also shipped.
6. **Pre-pow-at-join bearer grandfathering.** Tracked separately in `HARDENING.md` §2.1. Before pow-at-join lands, production will have some number of anonymous-cold-join bearers minted under the old free-token flow. The plan does not touch these; they keep working. HARDENING recommends flagging them as provisional and requiring re-verification on first sensitive use. Flag to Owen pre-launch: does prod actually carry risky free-minted bearers, or is the pre-launch real-human population small enough that the cleanest answer is "honor them, ship, move on"? This is a product call, not a technical one.
