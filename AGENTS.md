...
`src/` contains the TypeScript runtime. `server.ts` is the HTTP entrypoint, `app-contract.ts` holds shared app/repository types, and `app.ts` routes actions into domain handlers such as `app-admissions.ts`, `app-content.ts`, `app-messages.ts`, `app-profile.ts`, `app-system.ts`, and `app-updates.ts`. `postgres.ts` wires auth/runtime concerns, while `src/postgres/*.ts` holds repository modules by domain. Operational entrypoints include `token-cli.ts`, `http-smoke.ts`, `ai-smoke.ts`, and `ai-operator.ts`. `test/` mirrors runtime behavior with `*.test.ts` files. SQL migrations live in `db/migrations/` and use ordered numeric prefixes. Supporting material lives in `docs/`, shell automation in `scripts/`, queue state in `automation/progress-queue.json`, and deployment units in `ops/systemd/`.
...
- `npm run db:health` checks migration status, runtime role safety, and optionally hits `session.describe`.
