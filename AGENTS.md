...
`src/` contains the TypeScript runtime. `server.ts` is the HTTP entrypoint, `app-contract.ts` holds the shared app/repository types, `app.ts` holds top-level action plumbing, and domain handlers are split across `app-admissions.ts`, `app-content.ts`, `app-deliveries.ts`, `app-messages.ts`, `app-profile.ts`, and `app-system.ts`. `postgres.ts` handles auth/runtime wiring, while `src/postgres/*.ts` holds repository modules by domain. The other `*-cli.ts` files, `http-smoke.ts`, and `delivery-worker.ts` are operational entrypoints. `test/` mirrors runtime behavior with `*.test.ts` files. SQL migrations live in `db/migrations/` and use ordered numeric prefixes such as `0015_delivery_worker_tokens.sql`. Supporting material lives in `docs/`, shell automation in `scripts/`, queue state in `automation/progress-queue.json`, and deployment units in `ops/systemd/`.
