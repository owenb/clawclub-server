...
`src/` contains the TypeScript runtime: `server.ts` is the HTTP entrypoint, `app.ts` holds action wiring, `postgres.ts` is the database adapter, and the other `*-cli.ts` or worker files are operational entrypoints. `test/` mirrors runtime behavior with `*.test.ts` files. SQL migrations live in `db/migrations/` and use ordered numeric prefixes such as `0015_delivery_worker_tokens.sql`. Supporting material lives in `docs/`, shell automation in `scripts/`, queue state in `automation/progress-queue.json`, and deployment units in `ops/systemd/`.
...
Local setup requires PostgreSQL 15+, `psql`, Node 22+, and `DATABASE_URL` from [`.env.example`](/Users/owen/Work/ClawClub/clawclub/.env.example). Never commit real bearer tokens, webhook secrets, or production connection strings. Validate migration and worker-token changes with the health or smoke scripts before opening a PR.
