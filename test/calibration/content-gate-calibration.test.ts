...
async function expectProfilePass(id: number, patch: Record<string, unknown>): Promise<void> {
  const owner = await h.seedOwner(ownerSlug(id), ownerName(id));
  const result = await h.apiOk(owner.token, 'members.updateProfile', {
    clubId: owner.club.id,
    ...patch,
  });
...
describe('content gate merge-path regressions', () => {
  ...
  it('92. passes a members.updateProfile patch that replaces one field with filler while others stay specific', async () => {
    ...
    const updated = await h.apiOk(owner.token, 'members.updateProfile', {
      clubId: owner.club.id,
  ...
  it('93. rejects a members.updateProfile patch that removes the last substantive field', async () => {
  ...
  it('94. passes a members.updateProfile patch that adds concrete website and links to an already specific profile', async () => {
  ...
  it('95. rejects a members.updateProfile patch that adds illegal-service links', async () => {
