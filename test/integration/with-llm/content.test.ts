...
describe('events', () => {
  it('member creates an event and sees it in events.list', async () => {
    ...
    const created = await h.apiOk(member.token, 'content.create', {
      ...
      summary: 'Casual founders breakfast at The Table in Shoreditch from 8:30am to 10:00am BST. We will go around the room and each share one thing we are stuck on and one thing working well. Bring your own coffee order; breakfast is covered.',
      event: {
        location: 'The Table, 83 Southwark Street, London SE1',
        startsAt: '2026-05-01T08:30:00+01:00',
        endsAt: '2026-05-01T10:00:00+01:00',
  ...
  it('another member can RSVP and response is visible on the event', async () => {
    ...
    const created = await h.apiOk(organizer.token, 'content.create', {
      ...
      summary: 'We will review the latest design mockups for the member dashboard from 11:00am to 12:30pm BST, collect feedback, and prioritise the next sprint of UI work. Bring your laptop if you want to follow along in Figma. The Zoom link will be shared with confirmed attendees the day before.',
      event: {
        location: 'Zoom',
        startsAt: '2026-06-15T11:00:00+01:00',
        endsAt: '2026-06-15T12:30:00+01:00',
        timezone: 'Europe/London',
      },
    });
    const event = (created.data as Record<string, unknown>).content as Record<string, unknown>;
    ...
    const rsvpResult = await h.apiOk(attendee.token, 'events.setRsvp', {
      eventId,
      response: 'yes',
    });
  ...
  it('event listing shows the event to all club members', async () => {
    ...
    const created = await h.apiOk(creator.token, 'content.create', {
      ...
      summary: 'I raised a $2M seed round last year and am happy to share what I learned in an online office-hours session from 2:00pm to 3:30pm BST. Drop in with questions about pitch decks, term sheets, investor outreach, or anything else related to early-stage fundraising. The Google Meet link will be shared with RSVPs.',
      event: {
        location: 'Google Meet',
        startsAt: '2026-07-01T14:00:00+01:00',
        endsAt: '2026-07-01T15:30:00+01:00',
        timezone: 'Europe/London',
      },
    });
    const event = (created.data as Record<string, unknown>).content as Record<string, unknown>;
  ...
  it('events have proper fields: title, summary, startsAt, endsAt, timezone, capacity', async () => {
    ...
    const created = await h.apiOk(member.token, 'content.create', {
      ...
      event: {
        ...
        endsAt: '2026-07-10T17:00:00-04:00',
