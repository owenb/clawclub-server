...
export const LEAK_AUDIT_FIXTURES: Record<string, LeakAuditFixture> = {
  ...
  'clubs.applications.get': {
    buildInput: (ctx) => ({ applicationId: ctx.ownApplicationMembership.id }),
  ...
  'content.get': { buildInput: (ctx) => ({ threadId: ctx.threadId, limit: 20 }) },
  ...
  'invitations.list': { buildInput: (ctx) => ({ clubId: ctx.testClub.id }) },
  ...
  'messages.get': { buildInput: (ctx) => ({ threadId: ctx.dmThreadId, limit: 20 }) },
