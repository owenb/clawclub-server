...
describe('non-owner clubadmin access tracks live subscription unless manually comped', () => {
  it('promoted non-owner clubadmin loses access when their subscription lapses', async () => {
    const owner = await h.seedOwner('lapse-admin-club', 'Lapse Admin Club');
    const member = await h.seedPaidMember(owner.club.id, 'Lapse Admin');
    ...
    const before = await h.apiOk(member.token, 'clubadmin.clubs.getStatistics', {
      clubId: owner.club.id,
    });
    assert.equal(((before.data as Record<string, unknown>).stats as Record<string, unknown>).clubId, owner.club.id);

    await lapseSubscription(member.membership.id);

    const err = await h.apiErr(member.token, 'clubadmin.clubs.getStatistics', {
      clubId: owner.club.id,
    });
