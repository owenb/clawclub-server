 ...
 *   - log in,
 *   - not be a superuser,
 *   - hold CONNECT + CREATE on the target database plus USAGE + CREATE
 *     on schema public so init.sql and migrations can create / alter /
 *     drop objects under its ownership, including new app-owned schemas.
 *
...
async function assertProvisionedRoleHasExpectedShape({
  ...
  try {
    ...
    const databaseCreate = (await adminClient.query<{ has: boolean }>(
      `select has_database_privilege($1, $2, 'CREATE') as has`,
      [roleName, databaseName],
    )).rows[0]!.has;
    assert.equal(connectGranted, true, 'role should have CONNECT on the database');
    assert.equal(databaseCreate, true, 'role should have CREATE on the database');
  ...
  const probeSchema = `provision_schema_${suffix}`;
  try {
    await roleClient.query(`create schema ${quoteIdentifier(probeSchema)}`);
    ...
    await roleClient.query(`drop schema ${quoteIdentifier(probeSchema)}`);
  } finally {
    roleClient.release();
    await rolePool.end();
    ...
    try {
      ...
      await cleanupPool.query(`drop schema if exists ${quoteIdentifier(probeSchema)}`);
