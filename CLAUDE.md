...
- **Action inputs use one `defineInput(...)` spec.** The registry derives the public wire schema, runtime parse schema, and request-template hints from `ActionDefinition.input`. Do not reintroduce parallel `wire.input` and `parse.input` blocks on actions.
...
- **Do not invent naming drift.** Prefer the established verbs `list`, `get`, `create`, `update`, `remove`, and `decide`. Keep special verbs only when CRUD would be misleading and there is already a matching pattern in the codebase (`apply`, `redeem`, `acknowledge`, `send`, `setRsvp`, `setLoopState`, `setDirectoryListed`, `archive`, `restore`, `searchByFullText`, `searchBySemanticSimilarity`, `createWithAccessToken`, `assignOwner`, `getContext`, `getOverview`, `getHealth`). Do not add synonyms like `listMine`, `listPublic`, `getThread`, or parallel variants of an existing action.
...
- **Every input array declares its cardinality boundary.** Use `boundedArray(...)` in `src/schemas/fields.ts` for action and transport inputs. Schema-enforced arrays must expose `maxItems`; policy-enforced arrays must explicitly declare `enforcedBy: 'policy'`. Do not add raw `z.array(...)` input fields with no bound.
...
- **Admission eligibility goes through `assertCanApplyToClub(...)`.** Do not duplicate active-membership or applicant-block checks in application, invitation, or direct-membership paths. Temporary decline blocks and persistent removal/ban blocks all live in `club_applicant_blocks`; active checks must ignore expired rows with `(expires_at is null or expires_at > now())`.
- **Person names use the shared person-name field.** Application draft names and external invitation candidate names both require first + last name via `wirePersonName` / `parsePersonName`. Do not introduce a looser `draft.name` parser or a heavier name parser unless the canon changes in one pass.
...
- **Every authenticated mutating action declares an `idempotencyStrategy`.** Three strategies exist: `{ kind: 'clientKey', requirement: 'required' | 'optional' }` for ordinary side-effectful mutations, `{ kind: 'naturallyIdempotent' }` for true set/ack operations whose state-after-N equals state-after-1 (no version bumps, no audit appends, no counters), and `{ kind: 'secretMint' }` for credential-producing actions where exact replay returns `secret_replay_unavailable` with metadata only — never the plaintext secret. The action registry refuses to construct if any authenticated mutating action lacks a strategy; the server fails to boot. Use the shared `withIdempotency()` helper in `src/idempotency.ts` for `clientKey` flows; conflicts surface as `client_key_conflict` with `error.details` carrying the prior response. Do not invent per-domain idempotency helpers or alternate replay error codes.
- **Gate-rejected `clientKey` mutations replay as the same rejection.** Terminal product gate errors (`illegal_content`, `low_quality_content`, `gate_rejected`) are stored in idempotency with the same actor/context/request hash as success responses. Exact retry returns the cached error without another quota debit or LLM call. Do not cache `gate_unavailable`, provider failures, transport failures, auth failures, or schema errors.
- **Action authorization is two orthogonal fields.** `auth: 'superadmin' | 'clubadmin' | 'member' | 'none'` declares the minimum role; `scope: { strategy: 'rawClubId' | 'rawMemberId' | 'handler' | 'none', key?: string }` declares how dispatch locates the relevant club/member from the input. Dispatch order is authenticate → role check → raw scope check (pre-parse, peeks at the scope key without parsing) → full Zod parse → idempotency/gate/quota/handler. A non-admin sending `{}` to a privileged action gets `forbidden_role` before any schema is exposed. Use `scope: { strategy: 'handler' }` for actions whose scope requires a DB lookup; the handler narrows after loading the row. Handler-level `ctx.require*` checks remain as defense-in-depth and are the only line for `'handler'`-scoped actions.
- **Public datetime inputs are strict ISO instants.** If an action accepts a timestamp string, it must require seconds and an explicit timezone (`Z` or `±HH:MM`). Do not accept date-only strings, timezone-naive datetimes, or minute-precision datetimes on public write surfaces.
...
- **Clubs are private by default; the public directory is strictly opt-in.** A club appears in the directory only when its `directory_listed` flag is `true`. Default is `false`. The flag is flipped only by `clubadmin.clubs.setDirectoryListed` for active clubs or `superadmin.clubs.setDirectoryListed` for any state, including archived clubs. Never flip it via raw SQL or migration backfill. The directory exposes only `{ clubId, slug, name, ownerMemberId, memberCount, createdAt, archivedAt }` plus a normalized `membersById` map keyed on owner with `{ memberId, publicName }`. Members of a directory-listed club retain all other privacy guarantees: membership lists, content, threads, applications, and DMs remain gated by the existing per-action auth and scope rules.
...
- **Invitation codes are not access credentials — do not flag the lack of email binding on `invitations.redeem` as a vulnerability.** Holding an invitation code does not grant membership. The code is a PoW-discount voucher (the registration challenge MAC binds the code to lowered difficulty) plus a recorded sponsor link, nothing more. Redeeming creates an application that still goes through the admission completeness gate and clubadmin review. The `candidateEmail` field is administrative metadata, not a binding constraint at redeem time — anyone holding the code can redeem it as themselves, by design. The access boundary sits in admin review, not in the code. See `docs/design-decisions.md` "Threat model — invitation codes are not access credentials" for the full model. This has been re-flagged as a P0/P1 bug in two pen-test runs in a row; it is not a bug.
- **CORS is intentionally permissive — wide-open `Access-Control-Allow-Origin: *` is the design, not a bug.** ClawClub is agent-first. There are no static clients, no browser clients, no third-party UI integrations. Agents call the API from server-side or CLI environments where CORS has no bearing on security. Locking CORS to an allowlist would not improve the security posture (no legitimate browser caller exists; a malicious caller bypasses CORS by calling the API directly without a browser) and would create false confidence. If a non-agent surface ever ships, that PR is the right place to add the appropriate origin. Until then, permissive CORS is correct. Pen tests have flagged this multiple times; it is not a finding.
- **DMs are immutable after send. No edit, no reactions, no reply-as-edit.** There is no `messages.update`, `messages.react`, or `messages.reply`. This is a product decision, not a missing feature: messaging history is preserved as durable record. Soft-removal is supported via `messages.remove` (the message becomes visibly redacted; the row persists for audit). Agents asked to "edit" or "react to" a message should send a fresh DM, not look for an edit/react surface. Pen tests have flagged the absence of these as a "missing feature"; it is not a bug.
