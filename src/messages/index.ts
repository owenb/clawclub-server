...
import { AppError, type MentionSpan, type WithIncluded } from '../repository.ts';
import { withTransaction } from '../db.ts';
import { withIdempotency } from '../idempotency.ts';
import { enforceActionQuota, QUOTA_ACTIONS } from '../quotas.ts';
...
function scopeDmMessageIds(messageIds: readonly string[], threadParticipantIds: readonly string[]): Map<string, string[]> {
  return new Map(messageIds.map((messageId) => [messageId, [...threadParticipantIds]]));
}

async function loadDirectMessageParticipantsIncluded(
  client: Pool | import('pg').PoolClient,
  participantIds: readonly string[],
): Promise<ReturnType<typeof emptyIncludedBundle> | Awaited<ReturnType<typeof loadIncludedMembers>>> {
  const uniqueParticipantIds = [...new Set(participantIds)];
  return uniqueParticipantIds.length > 0
    ? loadIncludedMembers(client, uniqueParticipantIds)
    : emptyIncludedBundle();
}
...
export function createMessagingRepository(pool: Pool): MessagingRepository {
  return {
    async sendMessage({ senderMemberId, recipientMemberId, messageText, clientKey }) {
      const performSend = async (client: Pool | import('pg').PoolClient): Promise<WithIncluded<{ message: MessageSummary }>> => {
        ...
        const mentions = hasPotentialMentionChar(messageText)
          ? await resolveDirectMessageMentions(client, messageText, { threadParticipantIds })
          : [];
        ...
        if (!threadId) {
          ...
          if (!threadId) {
            throw new AppError('thread_not_found', 'Failed to find or create direct thread');
          }
        }
        if (createdThreadId) {
          await client.query(
        ...
        if (threadId === null) {
          throw new AppError('thread_not_found', 'Failed to resolve direct thread');
        }
      ...
      return withTransaction(pool, async (client) => {
        ...
        return withIdempotency(client, {
          clientKey,
          actorContext: `member:${senderMemberId}:messages.send`,
          requestValue: { recipientMemberId, messageText },
          execute: async () => ({ responseValue: await performSend(client) }),
        });
      });
    },

    async listDirectThreads({ memberId, limit }) {
      ...
      const { mentionsByMessageId } = await loadDmMentions(pool, latestVisibleMessageIds, {
        participantIdsByMessageId: new Map(
          result.rows
            .filter((row) => !row.latest_is_removed)
            .map((row) => [row.latest_message_id, [memberId, row.counterpart_member_id]]),
        ),
      });
    ...
    async listInbox({ memberId, limit, unreadOnly, cursor }) {
      ...
      const participantIncluded = await loadDirectMessageParticipantsIncluded(
        pool,
        [memberId, ...pageRows.map((row) => row.counterpart_member_id)],
      );
      ...
      return {
        ...
        hasMore,
        nextCursor,
        included: mergeIncludedBundles(participantIncluded, hydrated.included),
      };
    },
    ...
    async readThread({ memberId, threadId, limit, cursor }) {
      ...
      const participantIds = [memberId, counterpartMemberId];
      const participantIncluded = await loadDirectMessageParticipantsIncluded(pool, participantIds);
      ...
      return {
        ...
        hasMore,
        nextCursor,
        included: mergeIncludedBundles(
          participantIncluded,
          latestThreadMentions.included,
          pageMentions.included,
        ),
      };
    },
