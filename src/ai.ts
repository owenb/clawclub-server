...
export const CLAWCLUB_OPENAI_INPUT_MICRO_CENTS_PER_TOKEN = 20;
export const CLAWCLUB_OPENAI_OUTPUT_MICRO_CENTS_PER_TOKEN = 125;
...
export const CLAWCLUB_EMBEDDING_INPUT_MICRO_CENTS_PER_TOKEN = 2;
...
function estimateStubEmbeddingTokens(values: string[]): number {
  return values.reduce((sum, value) => sum + Math.ceil(value.length / 4), 0);
}
...
export async function embedQueryText(input: {
  ...
  if (isEmbeddingStubEnabled()) {
    return {
      ...
      usageTokens: estimateStubEmbeddingTokens([input.value]),
    };
  }
...
export async function embedWithTuple(input: {
  values: string[];
  model: string;
  dimensions: number;
  abortSignal?: AbortSignal;
  ...
  if (isEmbeddingStubEnabled()) {
    return {
      embeddings: input.values.map((value) => buildStubEmbedding(
        `${input.model}:${input.dimensions}:${value}`,
        input.dimensions,
      )),
      usageTokens: estimateStubEmbeddingTokens(input.values),
    };
  }
  ...
  const provider = createOpenAI({ apiKey });
  const model = provider.embedding(input.model);
