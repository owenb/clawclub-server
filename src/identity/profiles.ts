...
import { translate23505 } from '../db.ts';
import {
  AppError,
  type ClubProfile,
...
import { encodeCursor, safeHttpUrl } from '../schemas/fields.ts';
...
const clubProfileFieldsSchema = z.object({
  ...
  websiteUrl: safeHttpUrl.nullable(),
...
export async function listMemberProfiles(client: DbClient, input: {
  actorMemberId: string;
  targetMemberId: string;
  actorClubIds: string[];
  clubId?: string;
}): Promise<MemberProfileEnvelope | null> {
  const identity = await readMemberIdentity(client, input.targetMemberId);
  ...
  const profiles = await readClubProfiles(client, input.targetMemberId, visibleClubs.map((club) => club.club_id));
...
function profileFieldsEqual(left: ClubProfileFields, right: ClubProfileFields): boolean {
...
async function insertClubProfileVersion(client: DbClient, input: {
  ...
  try {
    ...
    translate23505(error, 'member_club_profile_versions_member_club_version_unique', 'version_conflict');
    throw error;
  }
...
export async function createInitialClubProfileVersion(client: DbClient, input: {
  ...
  const versionId = await insertClubProfileVersion(client, input);
...
export async function updateClubProfile(
  ...
  const performUpdate = async (client: DbClient): Promise<MemberProfileEnvelope> => {
    const current = await readCurrentClubProfile(client, actor.member.id, patch.clubId);
    if (!current) {
      throw new AppError('profile_not_found', 'Club profile not found for this membership');
    }

    const currentFields = normalizeClubProfileFields({
    ...
    if (!profileFieldsEqual(currentFields, fields)) {
      const versionId = await insertClubProfileVersion(client, {
        membershipId: current.membership_id,
        memberId: actor.member.id,
        clubId: patch.clubId,
        fields,
        creatorMemberId: actor.member.id,
        generationSource: 'manual',
      });
      await enqueueEmbeddingJob(client, versionId);
    }

    const updated = await listMemberProfiles(client, {
      actorMemberId: actor.member.id,
      targetMemberId: actor.member.id,
      actorClubIds: membershipScopes(actor.memberships).clubIds,
    });
    if (!updated) {
      throw new AppError('missing_row', 'Updated profile could not be reloaded');
    }
    return updated;
  };
  ...
  return withTransaction(pool, async (client) => {
    ...
    return withIdempotency(client, {
      ...
      actorContext: `member:${actor.member.id}:members.updateProfile`,
      requestValue: patch,
...
}): Promise<{ profile: ClubProfileFields; usage: { promptTokens: number; completionTokens: number } | null }> {
  const apiKey = process.env.OPENAI_API_KEY;
  if (!apiKey) {
    return { profile: fallbackClubApplicationProfile(input), usage: null };
  }
...
If the application does not justify a field, leave it null.
    ...
    return {
      profile: mergeClubApplicationProfileFallback(
    ...
    logger.warn('application_profile_generation_fallback', {
      errorName: error instanceof Error ? error.name : typeof error,
      applicantName: input.applicantName,
      clubName: input.club.name,
    });
    return { profile: fallbackProfile, usage: null };
  }
}
    ...
    `with params as (
     ...
     select
       ...
       case
         ...
         when lower(m.public_name) like params.like_query escape '\\' or lower(m.display_name) like params.like_query escape '\\' then 1.0
         else 0.0
       end as _rank
     from accessible_club_memberships acm
     ...
     where acm.club_id = $1
       ...
       and (
         ...
         or lower(m.public_name) like params.like_query escape '\\'
         or lower(m.display_name) like params.like_query escape '\\'
       )
       and (
         $5::float8 is null
         or case
     ...
     limit $4`,
    [input.clubId, input.actorMemberId, input.query, fetchLimit, cursorRank, cursorMemberId, likeQuery],
  );
