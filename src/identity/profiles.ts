...
export async function getMemberProfile(pool: Pool, actorMemberId: string, targetMemberId: string, actorClubIds: string[]): Promise<MemberProfile | null> {
  ...
  const result = await pool.query<ProfileRow>(
    `with target_scope as (
     ...
     left join clubs n on n.id = ts.club_id and n.archived_at is null
    ...
    `with scope as (
     ...
     select
       ...
       jsonb_agg(distinct jsonb_build_object('clubId', n.id, 'slug', n.slug, 'name', n.name))
         filter (where n.id is not null) as shared_clubs
     from scope s
     join members m on m.id = s.member_id and m.state = 'active'
     left join current_member_profiles cmp on cmp.member_id = m.id
     left join member_profile_versions mpv on mpv.member_id = m.id
       and mpv.version_no = cmp.version_no
     left join accessible_club_memberships anm2 on anm2.member_id = m.id
