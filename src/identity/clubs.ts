/**
 * Identity plane — club management.
 *
 * Clubs, club_versions, and ownership live in the identity database.
 */
  ...
  return withTransaction(pool, async (client) => {
    const clubResult = await client.query<{ club_id: string }>(
      `with owner_member as (
         select m.id from app.members m where m.id = $4 and m.state = 'active'
       ), inserted_club as (
         insert into app.clubs (slug, name, summary, owner_member_id)
...
export async function archiveClub(pool: Pool, input: ArchiveClubInput): Promise<ClubSummary | null> {
  return withTransaction(pool, async (client) => {
    const result = await client.query<{ club_id: string }>(
      `update app.clubs set archived_at = coalesce(archived_at, now()) where id = $1 returning id as club_id`,
      [input.clubId],
    );
...
export async function assignClubOwner(pool: Pool, input: AssignClubOwnerInput): Promise<ClubSummary | null> {
  return withTransaction(pool, async (client) => {
    ...
    await client.query(
      `insert into app.club_versions (club_id, owner_member_id, name, summary, admission_policy, version_no, supersedes_version_id, created_by_member_id)
    ...
    if (current.current_owner_member_id !== input.ownerMemberId) {
      await client.query(`select set_config('app.allow_club_membership_state_sync', '1', true)`);
      await client.query(
        `update app.club_memberships set role = 'member', sponsor_member_id = $3
