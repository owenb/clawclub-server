import type { LogApiRequestInput, Repository } from './repository.ts';
...
function serializeError(error: unknown): JsonValue {
  if (error instanceof Error) {
    ...
    if (maybeCode.cause !== undefined) {
      serialized.cause = normalizeValue(maybeCode.cause);
    }
    return serialized;
  }
...
export function fireAndForgetRequestLog(repository: Repository, entry: LogApiRequestInput): void {
  ...
  logApiRequest.call(repository, entry).catch((err) => {
    logger.error('request_log_failure', err, { actionName: entry.actionName, memberId: entry.memberId });
  });
}
