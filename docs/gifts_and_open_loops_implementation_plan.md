# Gifts and Open Loops — Implementation Plan v2

This plan supersedes the v1 draft. It incorporates a code review that surfaced several real correctness and privacy issues v1 missed. Read the **Must-hold invariants** section first — those are the cross-cutting truths every change has to respect, and they're the things v1 got wrong.
...
5. **Reopen must allow rematch.** The `signal_background_matches` unique constraint is `(match_kind, source_id, target_member_id)` (`db/init.sql:1160`), and `createMatch` does `ON CONFLICT DO NOTHING`. So if a loop was matched once and then closed (matches transitioned to `delivered` or `expired`), reopening doesn't help — the worker's next pass will silently skip every recipient because the unique constraint is occupied. **Reopen must `DELETE` historical match rows for `source_id = entity_id`**, not just expire them. Closing still expires; reopening clears the dedupe slate.
...
7. **Closed loops are visible only to the author.** `content.list` already accepts `actorMemberId` via the handler context — but the SQL at `src/clubs/entities.ts:307` doesn't currently use it for any filtering. When `includeClosed: true`, the SQL must filter to the actor's own entities only: `WHERE (open_loop IS NULL OR open_loop = true OR e.author_member_id = $actor)`. Without the actor filter, `includeClosed: true` is a privacy leak — any member can list everyone's closed asks/gifts/services in shared clubs. The semantic-search helper at `src/clubs/index.ts:459` also needs the same treatment, even though it doesn't currently take `actorMemberId`.
...
- **Idempotent close/reopen.** Closing an already-closed loop succeeds. Reopening an already-open loop succeeds. Standard idempotent semantics.
...
2. **New repo method `closeEntityLoop`**: takes `{ actorMemberId, accessibleClubIds, entityId }`. SQL filter:
   ```sql
   UPDATE entities e
   SET open_loop = false
   ...
   RETURNING e.id
   ```
   Returns the updated entity summary or `null` (which the handler maps to 404). The `open_loop IS NOT NULL` predicate enforces invariant 3 at the row level (can't close a non-loopable entity). After the update, also expire pending matches:
   ```sql
   UPDATE signal_background_matches
   SET state = 'expired'
   WHERE source_id = $entityId AND state = 'pending'
     AND match_kind IN ('ask_to_member', 'offer_to_ask')
   ```

3. **New repo method `reopenEntityLoop`**: same SQL filter as close, with `open_loop = true`. After the update, `DELETE` historical match rows (invariant 5):
   ```sql
   DELETE FROM signal_background_matches
   WHERE source_id = $entityId
     AND match_kind IN ('ask_to_member', 'offer_to_ask')
   ```
   This frees the unique constraint so the worker's next pass can produce new matches against the same recipients. Idempotent: reopening an already-open loop is a no-op (the row update sets it to the same value).
...
5. **`findEntitiesViaEmbedding`** (`src/clubs/index.ts:459`): add `actorMemberId` to the input. Add the same closed-loop filter as `listEntities`. Also add the `expires_at` filter for invariant 9:
   ```sql
...
4. **`content.searchBySemanticSimilarity`** (`src/schemas/entities.ts:312`): same — pass `actorMemberId` through to `findEntitiesViaEmbedding`. No `includeClosed` flag here (search only returns currently relevant content; the author can use list with `includeClosed` to see their history).
...
- A gift that gets a match, then is closed (matches expired), then is reopened, can match the same recipient again on the next worker pass (invariant 5 — would fail in v1 because of the unique constraint).
...
1. Should `closeLoop`/`reopenLoop` produce a `club_activity` row? Today the worker reacts to `entity.version.published` activity rows. Closing/reopening doesn't create a new version, so it doesn't naturally emit activity. The worker's open-loop guards (invariants 6 + delivery check) cover the matching path without needing a new activity row, so I'd say no — keep activity tied to versioning.

2. Do we want a `content.list` filter for "only my closed loops" specifically (for "show me everything I've resolved this month")? Could be a future small addition. Not in scope here.

3. Should we add a soft cap on gifts per member per club (e.g., 10) to prevent flooding? Probably not for v1 — quotas already cap creation rate, and the SKILL.md guidance discourages spamming. Watch for abuse and add later if needed.
