...
```sql
CREATE TABLE club_activity_rollups (
    id              short_id DEFAULT new_id() NOT NULL,
    club_id         short_id NOT NULL REFERENCES clubs(id),
...
CREATE INDEX club_activity_rollups_club_seq_idx
    ON club_activity_rollups (club_id, to_seq DESC);
```
...
```sql
CREATE TABLE digest_worker_progress (
    club_id         short_id NOT NULL REFERENCES clubs(id),
...
digest-worker.ts
  ...
  │   └── Log LLM usage via log_llm_usage()
...
```sql
CREATE TABLE digest_cursors (
    member_id       short_id NOT NULL REFERENCES members(id),
    club_id         short_id NOT NULL REFERENCES clubs(id),
