...
|---|---|---|
| `member_payment` | Member paid for a club membership | +operator share (enters 120-day hold) |
...
| `operator_requested_refund` | Operator requested a refund for a removed member | −refund amount (deducted from operator balance) |
| `discount_shortfall` | Operator share went to $0 due to discount | Informational (no negative entries — see discount cap) |

The operator's available balance at any point is the sum of all events. Monthly statements are generated by filtering events for the statement period. Audits trace any balance to its constituent events.
