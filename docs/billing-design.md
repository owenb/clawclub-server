...
The operator's **gross position** is the sum of all events, but the **payout-eligible balance** requires partitioning events by hold status — see derived balances below. Monthly statements are generated by filtering events for the statement period. Audits trace any balance to its constituent events.
