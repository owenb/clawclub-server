...
- code redemption failures are intentionally opaque. Malformed, unknown, expired, revoked, consumed, or unusable-for-this-caller codes return `invalid_invitation_code`; the redeem path does not distinguish "real but unavailable" from "not real" on the wire. Once a code has been accepted as usable, actor/application state errors such as `application_in_flight`, `application_blocked`, and `application_limit_reached` may still surface because they describe the caller's application state rather than code existence
...
- DM inbox read-state is durable on `dm_inbox_entries.acknowledged_at timestamptz` (nullable). The legacy boolean `acknowledged` column has been removed; unread/read queries and writes use `acknowledged_at` only.
...
- `clientKey` is scoped per-actor and may be further scoped by resource when an action's `getScopeKey` includes a club, application, content item, or thread id. Anonymous `accounts.register` is scoped by validated client IP rather than a single global namespace, and authenticated actions start from `actorContext = member:<memberId>:<action>`. Cross-actor `clientKey` collisions cannot occur, and a repeated literal key on a different resource is not necessarily a replay
